...
dbutils.fs.mkdirs("/FileStore/tables/streamDir")

for (i <- 1 to 40) {
    dbutils.fs.cp(s"dbfs:/databricks-datasets/structured-streaming/events/file-$i.json", s"/FileStore/tables/streamDir/file-$i.json")
}
...
val inputPath = "dbfs:/FileStore/tables/streamDir/"
...
val static_counts  = staticInputDF.groupBy($"action", window($"time", "1 hour")).count()
 

static_counts.createOrReplaceTempView("static_counts")
